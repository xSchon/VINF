...
# Main GUI which uses searcher.py to perform searches
sg.theme("DarkBlue13")


def find_fields_to_search(
    values: dict, search_names: list, filter_amount: int
) -> tuple:
    """
    Function reading the search field and dropdown arguments, reading what columns and how to filter.
    args:
        :dict values - Values collected via the GUI runtime
        :list search_names - names used for search purposes (CSV header column mapping)
        :int filter amount - how many search fields are filled, how many to filter

    returns:
        tuple containing two lists - fitlering columns and filtering words.
    """
    ...
    # Read all DropDowns (keys and values)
    ...
    # Map to be suitable with colum names in the index
...
def create_gui() -> None:
    """
    Main GUI function. Create GUI for searches and perform them, calling the backend in searcher.py
    """
    search_names = [
        "Team",
        "Player Name",
    ...
    search_arrays = 1  # Make variable amount of filters

    layout = [
        [sg.Text("Search", justification="center")],
        [
            sg.Text("Filters dependency: "),
            sg.Button("OR", key="type_button", button_color=("white", "blue")),
        ],
        [
            sg.Column(
                [
                    [
                        sg.Text("Field: "),
                        sg.Combo(search_names, key="combo1", readonly=True),
                        sg.Text("Filter: "),
                        sg.InputText(key="1", size=(15, 20)),
                    ]
                ],
                key="-Column-",
            )
        ...
        [
            sg.Button("Search"),
            sg.Button("Add filter"),
            sg.Button("Clear output"),
            sg.Button("Close"),
            sg.Text("   Results amount:"),
            sg.Combo(
                ["5", "10", "25", "50", "All"],
                readonly=True,
                key="amount",
        ...
        [
            sg.Output(size=(180, 40), key="outp")
        ],  # Output element to display search results
    ]
    ...
    while True:
                ...
                [
                    [
                        ...
                        sg.Combo(
                            search_names,
                            readonly=True,
                            key=f"combo{str(search_arrays)}",
                        ),
                        sg.Text("Filter: "),
                        sg.InputText(key=str(search_arrays), size=(15, 20)),
                    ]
                ],
            )
            continue
        ...
        search_fields, search_values = find_fields_to_search(
            values, search_names, search_arrays
        )
        if len(search_fields) > 0:
            strings_to_print = searcher.search_index(
                fields_to_search=search_fields,
                queries=search_values,
                results_amount=values["amount"],
                search_type=window["type_button"].GetText(),
                printable_cols=results_show,
            )
